...
# create new visa
def create_new_visa(db: Session, visa: VisaCreate):
    new_visa = RelationModel.Visa(name=visa.name, user_id=visa.user_id )
    db.add(new_visa)
    db.commit()
    db.refresh(new_visa)
    return new_visa
...
# get all visa 
def read_all_visa(db: Session, skip: int, limit: int):
    query = db.query(RelationModel.Visa).offset(skip).limit(limit).all()
    return query
